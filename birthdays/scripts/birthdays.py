...
class CelebrationMessages():
    ...
    def check_contact(self, person):
        try:
            ...
            # Country
            if addresses and addresses[0].get("country") != None:
                details['country'] = (addresses[0].get("country")).upper()
            else:
    ...
    def send_group_message(self, details):
        self.parent.logger.debug(f"Checking for Group messages for {details['name']}") 
        try:                    
